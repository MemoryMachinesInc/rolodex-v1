...
one would break the promise that the name describes the artifact.
"""
...
def read_config(path: Path) -> dict[str, str]:
    """Parse and validate the ``[paths]`` table.
    ...
    unknown_tables = sorted(set(document) - {"paths"})
    if unknown_tables:
        raise ValueError(f"{path}: unknown table(s) {unknown_tables}; expected [paths]")
...
def resolve(
    *,
    ...
    keys = read_config(config_path) if config_path is not None else {}
